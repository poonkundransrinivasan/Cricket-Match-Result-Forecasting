...
from02to22 = from02to22.drop(columns = ['match_id', 'start_date', 'date']) #is date specific data really useful?
...
lr.fit(X_train, 
          y_train)
predictLR = dtc.predict(X_test)
outcomeLR = pd.DataFrame ({'Actual': y_test, 'Predicted': predictLR})
outcomeLR['Actual'] = label_encoders[18].inverse_transform(outcomeLR['Actual'])
outcomeLR['Predicted'] = label_encoders[18].inverse_transform(outcomeLR['Predicted'])

key = (label_encoders[18].classes_)
