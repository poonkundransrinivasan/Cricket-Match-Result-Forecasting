...
from02to22 = from02to22.drop(columns = ['match_id', 'start_date', 'date', 'winner', 'cricsheet_id', 'season']) #is date specific data really useful? also drop continuous match identifiers. We want the match stats
corr = sns.pairplot(from02to22)
corr
...
outcomeLR['Actual'] = label_encoders[17].inverse_transform(outcomeLR['Actual'])
outcomeLR['Predicted'] = label_encoders[17].inverse_transform(outcomeLR['Predicted'])

key = (label_encoders[17].classes_)
...
outcomeRFC['Actual'] = label_encoders[17].inverse_transform(outcomeRFC['Actual'])
outcomeRFC['Predicted'] = label_encoders[17].inverse_transform(outcomeRFC['Predicted'])
